...
def generate_random(table):
    ...
    while True:
        new_id = ''
        new_id = new_id.join(random.choice(string.ascii_lowercase)
                             for i in range(2))
        new_id = new_id.join(random.choice(string.ascii_uppercase)
                             for i in range(2))
        new_id = new_id.join(random.choice(string.digits) for i in range(2))
        new_id = new_id.join(random.choice(string.punctuation)
                             for i in range(2))
